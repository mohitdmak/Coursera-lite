...
h.prototype = {
  ...
  _onSliderReady : function() {
    ...
    if (j.settings.preserveTargetSize) {
      ...
      if (j.settings.responsive) {
        d(f).on("resize", function() { j._preserveTargetSize() })
      }
    }
    ...
    for (var k = 0; k < j.slides.length; k++) {
      d('<li data-num="' + k +
        '"><button class="pogoSlider-nav-btn"></button></li>')
          .appendTo(l)
          .on("click", function() { j.toSlide(d(this).data("num")) })
    }
  },
  getAppliedProps : function(o) {
    var y = g.styleSheets;
    var m = new RegExp("{(.+)}");
    o.matches = o.matches || o.webkitMatchesSelector || o.mozMatchesSelector ||
  ...
  pause : function() {
    if (this.settings.autoplay) {
      ...
      for (var j = 0; j < this.slides[this.currentSlideIndex].children.length;
           j++) {
  ...
  resume : function() {
    if (this.settings.autoplay) {
      ...
      if (this.slideTimeRemaining > 0 && !this.navigating) {
        if (this.settings.displayProgess) {
          this.$element.find(".pogoSlider-progressBar-duration")
  ...
  _onSlideStart : function(k) {
    ...
    if (this.settings.autoplay) {
      ...
      if (this.settings.displayProgess && !this.paused) {
        this.slides[k]
            .$element.find(".pogoSlider-progressBar-duration")
            .css("width", "0")
            .animate({"width" : "100%"}, this.slideTimeRemaining, "linear")
      }
    }
    ...
    if (this.paused) {
      for (var j = 0; j < this.slides[k].children.length; j++) {
        this.slides[k].children[j].$element.precss("animation-play-state",
                                                   "paused")
      }
    }
    if (this.settings.generateNav) {
      this.$element.find(".pogoSlider-nav-btn")
  ...
  _onSlideEnd : function(l) {
    ...
    if (this.paused) {
      ...
      k = this.slides[l].totalSlideDuration -
          (this.slideTimeRemaining - ((new Date()) - this.slideStartTime))
    }
    if (this.slides[l].children.length > 0 &&
  ...
  _slideElementsTransitionOut : function(l) {
    for (var j = 0; j < this.slides[l].children.length; j++) {
      var k = this.slides[l].children[j];
      k.$element.precss("animation-delay", "")
  ...
  _slideCleanup : function(k, j) {
    ...
    this.slides[k]
        .$element.attr("style", this.slides[k].$element.data("original-styles"))
        .css("opacity", j ? "1" : "0")
  },
  ...
  _createSlideSlices : function(A, q, r) {
    ...
    if (this.paused) {
      ...
      m = this.slides[A].totalSlideDuration -
          (this.slideTimeRemaining - ((new Date()) - this.slideStartTime))
    }
    if (A === this.prevSlideIndex && this.slides[A].children.length > 0 &&
    ...
    C.children()
        .wrapAll('<div class="pogoSlider-slide-slice" style="' +
                 "width:" + n + "%;height:" + l + "%;top:0%;left:0%;" +
                 '"/>')
        .wrapAll('<div class="pogoSlider-slide-slice-inner" style="' + B +
                 "width:" + t + "%;height:" + s + "%;top:0%;left:0%;" +
                 '"/>');
    C.attr("style",
           function(j, D) { return D.replace(/(?:background)[^;]+;/g, "") });
    for (var w = 0; w < p; w++) {
      var o = w % q;
      var z = Math.floor(w / q);
      var u = "width:" + n + "%;height:" + l + "%;top:" + (l * o) +
              "%;left:" + (n * z) + "%;";
      var v = "width:" + t + "%;height:" + s + "%;top:-" + (100 * o) +
              "%;left:-" + (100 * z) + "%;";
      var y = "";
      if (this.settings.preserveTargetSize) {
        y = "background-size:" + this.$element.width() + "px " +
            parseFloat(this.$element.css("padding-bottom")) + "px;";
        console.log(y)
      }
  ...
  slideTransitions : {
    fade : function(j, l) {
      ...
      this.slides[j].$element.precss(
          {"*opacity" : "0", "transition-duration" : k.duration + "ms"});
      k.$element.precss(
          {"*opacity" : "1", "transition-duration" : k.duration + "ms"})
    },
    slide : function(j, k) {
      var l;
    ...
    slideLeft : function(k, m) {
      ...
      var l = j.slides[m];
      j.slides[k]
          .$element.precss("animation-duration", l.duration + "ms")
      ...
      return function() {
        ...
        l.$element.attr("style", l.$element.data("original-styles"))
            .css("opacity", "1")
    ...
    slideOverLeft : function(j, l) {
      ...
      k.$element
          ...
          .addClass("pogoSlider-animation-leftIn");
      return function() {
        k.$element.attr("style", k.$element.data("original-styles"))
            .css("opacity", "1")
            .removeClass("pogoSlider-animation-leftIn")
      }
    },
    ...
    foldRight : function(l, k) {
      ...
      m.$element.css(
          {"overflow" : "visible", "z-index" : s.settings.baseZindex});
      q.$element.css({
        "opacity" : 1,
        "overflow" : "visible",
      ...
      n.addClass("pogoSlider-animation-foldInRight")
          .precss("animation-duration", q.duration + "ms");
      j.addClass("pogoSlider-animation-foldOutRight")
          .precss("animation-duration", q.duration + "ms");
      return function() {
    ...
    foldLeft : function(l, k) {
      ...
      p.addClass("pogoSlider-animation-foldInLeft")
          .precss("animation-duration", q.duration + "ms");
      o.addClass("pogoSlider-animation-foldOutLeft")
          .precss("animation-duration", q.duration + "ms");
      return function() {
        s.$element.css("overflow", "");
        s._slideCleanup(k, true)
  ...
  compatSlideTransitions : {
    ...
    slideLeft : function(j, l) {
      ...
      k.$element.css({left : "100%", "opacity" : 1})
    ...
    slideUp : function(j, l) {
      ...
      k.$element.css({top : "100%", "opacity" : 1})
    ...
    slideRevealLeft : function(j, l) {
      var k = this.slides[l];
      this.slides[j]
          .$element.css("z-index", this.settings.baseZindex + 1)
